...
app.get("/filter", (req, res) => {
  ...
  if (req.query.genre === "Type" && req.query.regionbased !== "Region") {
    eventModel.find({ region1: r }, function (err, events) {
      res.render("participant", {
        eventList: events,
      });
    });
  } else if (req.query.regionbased === "Region" && req.query.genre !== "Type") {
    eventModel.find({ eventType1: req.query.genre }, function (err, events) {
      res.render("participant", {
        eventList: events,
      });
    });
    ...
    eventModel.find(
      { eventType1: req.query.genre, region1: r },
      function (err, events) {
        res.render("participant", {
          eventList: events,
        });
...
app.get("/s", (req, res) => {
  ...
  eventModel.find(
    { eventName1: { $regex: sname, $options: "i" } },
    function (err, events) {
      res.render("search", {
        eventList: events,
      });
...
app.get("/dashboard", async (req, res) => {
  ...
  const [count, countuser, display] = await Promise.all([
    count1,
    count2,
    display1,
  ]);
